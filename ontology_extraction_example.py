...
def main():
    ...
    ontology_path = Path("foaf.owl")
    if not ontology_path.exists():
        print(f"\nError: Ontology file not found at {ontology_path}")
        print("Please ensure the foaf.owl file is in the current directory.")
        return
        
    print(f"\nFound ontology file: {ontology_path}")
...
and science.
    ...
    try:
        # Initialize pipeline with ontology
        print("\nInitializing pipeline with ontology...")
        pipeline = KnowledgeGraphPipeline(ontology_path=ontology_path)
        ...
        if success:
            ...
            # Optionally save the JSON-LD output to a file
            if result and 'jsonld' in result:
                output_file = Path("extracted_knowledge.jsonld")
                with open(output_file, 'w') as f:
                    import json
                    json.dump(result['jsonld'], f, indent=2)
                print(f"\nSaved JSON-LD output to {output_file}")
        else:
            print(f"Error processing text: {error}")
            
    except Exception as e:
        print(f"\nError: {str(e)}")
        print("Make sure the ontology file is valid and all dependencies are installed.")
